    ...
    <PageContainer>
      <div className="w-full max-w-5xl mx-auto">
        <div className="rounded-xl border border-slate-700/60 bg-slate-800/60 p-8 shadow-sm">
          <h1 className="text-2xl font-bold text-white mb-1 tracking-tight">Painel do Professor</h1>
            ...
            {mockVagas.map(vaga => (
              <div key={vaga.id} className="rounded-lg border border-slate-700/60 bg-slate-700/40 p-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Link to={`/professor/vaga/${vaga.id}/applicants`} className="bg-blue-500 hover:bg-blue-400 text-white px-3 py-2 rounded-md text-sm shadow-sm">Ver Candidatos</Link>
                </div>
              </div>
            ))}
          </div>
