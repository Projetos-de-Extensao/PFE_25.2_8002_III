    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-700/60 bg-gradient-to-r from-slate-900 to-slate-800/95 backdrop-blur">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-2">
          ...
          {/* Mobile: hamburger toggles menu (hidden on desktop via CSS) */}
                  ...
                  <nav className="flex flex-col gap-2">
                    {isStudent && (
                      <>
                        ...
                        <button onClick={() => { if (isBrowser) window.localStorage.removeItem('userRole'); navigate && navigate('/'); setMenuOpen(false) }} className="text-left px-4 py-3 rounded text-slate-900 hover:bg-slate-100">Sair</button>
                      </>
                    )}
                    {isProfessor && (
                      <>
